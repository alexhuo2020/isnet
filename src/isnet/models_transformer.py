"""
Code from Andrej karpathy's implementation of transfomer and GPT https://github.com/karpathy/nanoGPT/master/model.py
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            att = torch.tanh(att)#F.softmax(att, dim=-1)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.lm_head = nn.Linear(config.n_embd, config.out_d, bias=False)
    ...
    def forward(self, idx, targets=None):
        device = idx.device
        idx = idx.view(len(idx), 1, len(idx[0]))
        # b, t = idx.size()
